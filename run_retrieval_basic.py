...
def run_retrieval(exp, master, df, out_dir):
    ...
    df["atomic_context_groundtruth"] = df["atomic_context_groundtruth"].fillna("")

    gt_embs = embedder.encode(df["atomic_context_groundtruth"].tolist(), normalize_embeddings=True, batch_size=2)
    ...
    for i, row in df.iterrows():
        ...
        q_emb = q_embs[i]
        gt_emb = gt_embs[i]
        gt_text = row["atomic_context_groundtruth"]
        ...
        # RANKING FINAL SETELAH RERANKER
        if gt_chunk in retrieved:
            rank = list(retrieved).index(gt_chunk) + 1
        else:
            rank = top_k + 1   # fallback jika GT tidak ada dalam top K
